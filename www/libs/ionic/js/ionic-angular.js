 ...
 *
 * While we find using the custom Scroll features in Ionic in most cases, sometimes
 ...
 * menu button. We find that you also use the `enable-menu-with-back-views="false"`
 ...
 *
 * We find always using `primary` and `secondary`, so the buttons correctly map
 ...
 * we'd find using `side="left"`, so it's always on the left, no matter the platform.
 *
 ...
 *
 * Note: We do not find using [resolve](https://github.com/angular-ui/ui-router/wiki#resolve)
